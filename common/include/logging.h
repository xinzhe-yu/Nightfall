...
#define LOG_DEBUG(fmt, ...)                                                                        \
    do {                                                                                           \
        log_write(LOG_LEVEL_DEBUG, __FILE__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__);            \
    } while (0)
...
#define LOG_INFO(fmt, ...)                                                                         \
    do {                                                                                           \
        log_write(LOG_LEVEL_INFO, __FILE__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__);             \
    } while (0)
...
#define LOG_WARN(fmt, ...)                                                                         \
    do {                                                                                           \
        log_write(LOG_LEVEL_WARN, __FILE__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__);             \
    } while (0)
...
#define LOG_ERROR(fmt, ...)                                                                        \
    do {                                                                                           \
        log_write(LOG_LEVEL_ERROR, __FILE__, __LINE__, fmt __VA_OPT__(, ) __VA_ARGS__);            \
    } while (0)
